...
- Установленная Java 21+ (в разработке была Java 25) с настроенными переменными окружения
...
application.conf используется по умолчанию
...
В этом случае (для контейнеров) будет использоваться application-docker.conf 
```bash
# 1) собираем fat-jar
./gradlew clean shadowJar
