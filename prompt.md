...
Этап 5: упаковка приложения в Docker с соответствующим Dockerfile и настройка  docker-compose.yml для запуска приложения в связке с субд

Промпт:
```text
Ты — опытный backend/DevOps инженер.

Есть готовое Ktor-приложение на Kotlin, проект собирается через Gradle.
Есть PostgreSQL, который уже работает в Docker-контейнере.

Нужно:
1. Подключить приложение к PostgreSQL в Docker.
2. Запустить само приложение тоже в Docker-контейнере.
3. Сделать общий docker-compose.yml для удобного запуска.
4. Обеспечить доступ к приложению из браузера.
5. Учесть, что контейнеру приложения нужен доступ в интернет.

Предложи несколько вариантов решения, но сначала покажи самый простой и практичный для локальной разработки.

Объясни:
- нужен ли Dockerfile и как его сделать;
- как использовать Ktor Gradle plugin для сборки Docker-образа или почему лучше обычный Dockerfile;
- как должны выглядеть настройки подключения к PostgreSQL;
- как приложение и база должны видеть друг друга в docker-compose;
- какие порты надо пробросить;
- какие типичные ошибки бывают.

Если для ответа не хватает данных — задай уточняющие вопросы.
```
